...
def encode(image_src, message_src):
    ...
    maximumSizeMessage = 1500 # There is around 2100 pixel diagonally
    ...
    i = 0
    for x in range(0,1500): # There is around 2100 pixel diagonally
        pixel = list(img.getpixel((x, x)))
        if (i < len(bits_message)):
            pixel[2] = pixel[2] & ~1 | int(bits_message[i])
            i  = i + 1
