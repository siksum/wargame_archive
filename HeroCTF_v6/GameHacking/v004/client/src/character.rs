use macroquad::{
    color::Color, math::{vec2, Rect, Vec2}, prelude::coroutines::start_coroutine, shapes::draw_line, texture::{draw_texture_ex, DrawTextureParams, Texture2D}, window::{screen_height, screen_width}
};
use macroquad_platformer::{Actor, World};
use macroquad_tiled::Map;

use crate::{client, enemy::Enemy};
...
pub struct Character 
{
    // Game values
    pub hp: u8,
    pub mp: u8, // This time, will be used as id
    pub turn_angle: u32, 
...
impl Character 
{
    ...
    pub async fn attack_one(&mut self, enemy: &mut Character, world: &mut World) 
    {
        ...
        for point in 0..3
        {
            ...
            if position == attack_position {
                ...
                enemy.take_damage(world, self).await;
            }
        }
    ...
    pub async fn attack(&mut self, enemies: &mut Vec<Enemy>, world: &mut World) 
    {
        ...
        for point in 0..3
        {
            ...
            for enemy in enemies.iter_mut() 
            {
                ...
                if position == attack_position {
                    //println!("Damage taken");
                    enemy.char.take_damage(world, self).await;
                }
    ...
    pub async fn take_damage(&mut self, world: &mut World, from: &Character) 
    {
        //println!("Taking damage !!!");

        if self.sprite.contains("Wizar")
        {
            client::user_take_damage(from.attack_damage as u8).await;
            let new_hp = client::get_user_life().await;
    
            self.hp = new_hp;
        }
        else if self.sprite.contains("Torch") {
            let new_hp = client::user_make_damage(self.mp).await;
            self.hp = new_hp;
        }
        else    // treasure
        {
            client::treasure_dead().await;
        } 
        if self.hp <= 0
        {
            return;            
    ...
    // Seriously dude ?
    pub async fn position_smoother(&mut self, world: &mut World) 
    {
        ...
        if move_x || move_y 
        {
            if self.sprite == "Wizar".to_string()
            {
                start_coroutine(client::send_user_position(world.actor_pos(self.collider)));
            }
    ...
    pub fn draw_character(&mut self, map: &Map, pos: Vec2) 
    {
        ...
        // attack selector isn't used (it could've been with 2 different attack animations)
        // Leaving it in because as we say in french "ça te fera la bite"
        else { 
        current_sprite = {
            if self.action == CharacterActionState::Idle 
            {
