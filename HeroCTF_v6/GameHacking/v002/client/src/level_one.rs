...
use crate::{
    character::{Character, CharacterActionState}, client, enemy::Enemy, misc::random_point_in_outer_square, questing::QuestLog, GameState
};
...
pub async fn level_one_draw(
    ...
    if character_state.action == CharacterActionState::Damaging {
        character_state.attack(spawned_enemies, world);
        character_state.attack(treasure, world);
    ...
    {
        client::server_send_game_end_time().await;
        *game_state = GameState::Flag;
    }

    character_state.apply_displacement(world);
    character_state.position_smoother(world);
