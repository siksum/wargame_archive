...
pub async fn server_connect() -> u8 {
    ...
    // 0x.. -> Game version
    // 0xFF -> Null
    let w = stream.write(&[0xFA, 0xF0, 0x05, 0xFF]).unwrap();
...
{
    USER_STREAM.lock().await.get(0).unwrap().write_all(&[0xFA, 0xF0, 0x05, 0xFE]).unwrap();
}
...
pub async fn boon(boon_num : u8)
{
    let mut command_buffer = vec!(0x7u8);

    USER_STREAM.lock().await.get(0).unwrap().write_all(&command_buffer).unwrap();
    USER_STREAM.lock().await.get(0).unwrap().write_all(&[boon_num]).unwrap();
...
pub async fn user_make_damage(enemy_index : u8) -> u8
{
    ...
    let mut command_buffer = vec!(0xAu8);
    USER_STREAM.lock().await.get(0).unwrap().write_all(&command_buffer).unwrap();
    USER_STREAM.lock().await.get(0).unwrap().write_all(&[enemy_index]).unwrap();

    let reponse = read_from_stream().await;
    let new_hp = reponse.as_bytes().get(0).unwrap_or(&0);
...
pub async fn receive_enemy(enemy_vector : &Vec<Enemy>) -> [u8 ; 4]
{
    let mut command_buffer = vec!(0x8u8);

    USER_STREAM.lock().await.get(0).unwrap().write_all(&command_buffer).unwrap();
    let mut response_buffer = [0u8; 0x4];
    let n = USER_STREAM
        .lock()
        .await
        .get_mut(0)
