...
async fn create_enemy_vector(world : &mut World) -> Vec<Enemy>
{

    let boss_nmi = (vec2(122., 28.), 200);
...
async fn main() 
{
    ...
    let nmi_buffer = client::receive_enemy(&Vec::<Enemy>::new()).await;
    let mut nmi_boss = Enemy::make_enemy(&mut world, vec2(60., 55.), "Torch_Red".to_string());
    nmi_boss.char.mp = *nmi_buffer.get(0).unwrap();
    nmi_boss.char.hp = *nmi_buffer.get(1).unwrap();
    nmi_boss.char.attack_damage = *nmi_buffer.get(2).unwrap();
    let mut last = (5., 27.);
    let mut nmi_vec = Vec::<Enemy>::new();
    let mut treasure_vec = Vec::<Enemy>::new();
    nmi_vec.push(nmi_boss);
