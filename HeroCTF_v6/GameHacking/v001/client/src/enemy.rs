...
impl Enemy 
{
    ...
    pub fn create_enemy(world: &mut World, positions_and_hp: Vec<(Vec2, u8)>) -> Vec<Enemy> {
        ...
        for (position, hp) in positions_and_hp {
            // println!("Creating enemy at {:?}", position);
            let sprite = {
                if hp >= 15
                {
                    "Torch_Red"
            ...
            enemy.char.attack_damage = rand::thread_rng().gen_range(5..25) + enemy.char.hp;
            enemy.char.hp = hp;
            enemies.push(enemy);
        }
        enemies
    ...
    pub fn aggro(&mut self, world : &mut World, target : &mut Character)
    {
        ...
        // no movement will be added because the position will not be the next final pos
